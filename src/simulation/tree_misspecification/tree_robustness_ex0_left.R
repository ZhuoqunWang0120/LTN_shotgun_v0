...
# fit LTN with left-skewed tree
niter = 10^4
yyl = leaf2node_special(cnt, 'left')
if (!file.exists(paste0(cachedir,'gibbs_left_lambda',lambda,'K',K,'_',id,'.rds'))){
gibbs1 = gibbs_ltn_hp_cov_mu(niter=niter,YL=yyl$leftval,Y=yyl$nodeval,SEED=1,lambda=lambda)
saveRDS(gibbs1, paste0(cachedir,'gibbs_left_lambda',lambda,'K',K,'_',id,'.rds'))
...
sigma=solve(omega)
saveRDS(list(mu_left=mu,sigma_left=sigma), paste0(cachedir,'est_para_left_lambda',lambda,'K',K,'_',id,'.rds'))
